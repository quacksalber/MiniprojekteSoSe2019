...
public class Aufgabe2 {
	...
	public static int convertColorToGrayscale(int rgb) {
		// TODO: implement me!
		int gray = (Colors.getR(rgb) + Colors.getG(rgb) + Colors.getB(rgb)) / 3;
		return Colors.toRGB(gray, gray, gray);
	}
	...
	public static int[][] andyWarhol(int[][] pixels) {
		...
		for (int i = 0; i < copy.length; i++) {
			for (int j = 0; j < copy[i].length; j++) {
				if (i < copy.length / 3) {
					copy[i][j] = Colors.toRGB(Colors.getR(pixels[i][j]), 0, 0);
				} else if (copy.length / 3 <= i && i < (copy.length * 2) / 3) {
					copy[i][j] = Colors.toRGB(0, Colors.getG(pixels[i][j]), 0);
				} else
					copy[i][j] = Colors.toRGB(0, 0, Colors.getB(pixels[i][j]));
			}
		}
		return copy;
	}
	 ...
	 * @param borderWidth
	 *            The border width in number of pixels (>= 0).
	 * @param borderColor
	 *            The border color.
		...
		for (int i = 0; i < copy.length; i++) {
			for (int j = 0; j < copy[i].length; j++) {
				if ((i <= borderWidth || i > pixels.length - borderWidth)
						|| (j <= borderWidth || j > pixels[i].length - borderWidth)) {
					borderImage[i][j] = borderColor;
				} else {
					borderImage[i][j] = pixels[i][j];
				}
			}
		}
	 ...
	 *            The pixels of the picture.
	 * @param borderWidth
	 *            The width of the border in pixels (>= 0).
	 * @param borderColor
	 *            The border color.
	 ...
	 * 
	 * @param length
	 *            The length of the axis (>0)
	 * @param n
	 *            The interval of the pixels (>0).
	 ...
	 * @param n
	 *            The n to define how much to shrink the picture.
...
//		 Images.writeImage(name + "-rotate-" + timestamp + extension,
//		 rotatePicture(Images.readImage(input)));
//		 Images.writeImage(name + "-shrink-" + timestamp + extension,
//		 shrinkPicture(Images.readImage(input), 3));
//		 Images.writeImage(name + "-smooth-" + timestamp + extension,
//		 shrinkPictureSmooth(Images.readImage(input), 3));
	}
